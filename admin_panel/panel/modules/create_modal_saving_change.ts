export const remove_modal_update_changes = (shadowRoot:ShadowRoot)=>{
    try {
        if(shadowRoot.querySelector('#saving_changes_id'))
        shadowRoot.querySelector('#saving_changes_id')?.remove()
    } catch (error) {
        
    }
}

export const createModal_update_changes = (shadowRoot:ShadowRoot,text:string=`Trwa ładowanie zmian`)=>{
